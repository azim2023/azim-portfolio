    ...
    <div className="max-w-10xl mx-auto w-full bg-white/80 backdrop-blur-lg shadow-xl rounded-2xl p-10 mb-10 transition-transform duration-500 hover:scale-[1.01] border border-slate-200 hover:shadow-brand-glow">
      <div className="flex flex-col md:flex-row gap-8 md:gap-16 items-center">
        ...
        <div className="w-full md:flex-1 flex flex-col justify-between self-stretch">
          <div>
            <h3 className="text-3xl lg:text-4xl font-bold text-slate-800 italic [text-shadow:1px_1px_3px_rgba(0,0,0,0.1)]">
              {project.title}
            </h3>
          </div>

          <div>
            <p className="text-base lg:text-lg text-slate-600 my-6 leading-relaxed">
              {project.blurb}
            </p>
          </div>
          ...
          <div>
            <ul className="flex flex-wrap gap-3 mb-6">
              {project.tech.map((t) => (
                <li key={t} className="px-3 py-1 bg-slate-100 text-slate-700 font-medium rounded-full text-sm">
                  {t}
                </li>
              ))}
            </ul>
            ...
            <div className="flex gap-6">
              {project.github && (
                <a
                  ...
                  className="inline-flex items-center gap-2 text-slate-500 font-medium hover:text-brand transition-colors duration-300"
                >
                  <FiGithub size={18} />
