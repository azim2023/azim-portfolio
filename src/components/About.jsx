...
export default function About() {
  ...
  const [activeYear, setActiveYear] = useState(t.about.timelineData[0].year);
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    scrollTimeout.current = setTimeout(() => {
        ...
        let currentYear = t.about.timelineData[0].year;
        for (let i = 0; i < sections.length; i++) {
            const section = sections[i];
            if (section.getBoundingClientRect().top < window.innerHeight / 2) {
                currentYear = t.about.timelineData[i].year;
            }
        }
  ...
  const activeIndex = t.about.timelineData.findIndex(item => item.year === activeYear);
  ...
  return (
    <section id="about" className="max-w-3xl mx-auto px-4 py-24 cursor-default scroll-mt-16">
      <div className="mb-12">
        ...
        <p className="mb-4 text-center">{t.about.subtitle}</p>
      </div>
      ...
      <nav className="sticky top-16 z-30 mb-8 md:hidden bg-slate-100/80 backdrop-blur-md rounded-lg shadow-sm">
        <div className="relative flex justify-center items-center gap-4 sm:gap-8 py-3">
          {t.about.timelineData.map((item, index) => (
            <a
              key={item.year}
